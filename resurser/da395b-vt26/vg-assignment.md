...
- API: [https://coinmarketcap.com/api/](https://coinmarketcap.com/api/).
- [React Bootstrap](https://react-bootstrap.netlify.app/) för att designa webbapplikationen.
...
- API: [https://swapi.info/](https://swapi.info/).
- [React Bootstrap](https://react-bootstrap.netlify.app/) för att designa webbapplikationen.
...
Så länge ni skapar bra branches, har tydliga meddelande i era commits och **inte** publicerar all kod i slutet av uppgiften (när allt är klart) så har ni med största sannolikhet god versionshanteringsmetodik. Ni lämnar sedan bara en länk till er GitHub-sida på Canvas vid inlämning. Tänk på att utforma er versionshantering så att vi tydligt kan se den i **efterhand**.

---

### Del 3: Pakethantering

Ni ska använda er av pakethanteraren t.ex. [NPM](https://www.npmjs.com/) för att hämta era externa bibliotek. Tänk på att i er `.gitignore` exkludera kataloger så som `node_modules` så alla externa bibliotek ni är beroende av **inte** hamnar på GitHub.

När ni skapar ert repository på GitHub kan ni välja att inkludera en `.gitignore` baserat på ett programmeringsspråk, välj då t.ex. `node` så får ni en bra utgångspunkt.
{: .info}
...
Den här gången lämnar du in din inlämning direkt på Canvas. Du skickar helt enkelt in länken till ditt GitHub-repo. Glöm inte att skicka med ev. API-nycklar som en kommentar på Canvas så att vi har möjlighet att testköra projektet när vi rättar.
