...
![Bild på tabeller](../images/20251209_145937.jpg)
...
**OM NI TITTAR PÅ FÖRELÄSNINGEN NEDAN** Tänk på att det är från tidigare version av kursen och att ev. kursinfo **INTE ÄR AKTUELL** (t.ex. deadlines, information om inlämningsuppgifter, etc.).
...
def main():
    ...
    while True:
        ...
        if choice == "1":
            ...
            print("Du har angett ett felaktiv val, försök på nytt!")

def header(text):
    """Skriver ut en rubrik med given text
    
    Args:
        text (str) : Rubrikens text
    """
    print("*"*50)
    print(text)
    print("*"*50)

def search_for_products(list_of_products):
    """Söker efter produkter och skriver ut sökresultatet
    
    Args:
        list_of_products (list): Lista på produkterna
    """
    header("Sök efter produkter")
    ...
    if len(search_results) == 0:
        print("Ingen sökträff!")
    else:
        print_products(search_results, "Sökresultat")

def remove_product(list_of_products):    
    """Tar bort en product
    
    Args:
        list_of_products (list): Lista på produkterna
    """
    header("Radera produkt")
    id = input("Id: ")
    ...
    if product_removed:
        print("Produkten raderad!")
    else:
        print(f"Det fanns ingen produkt med id: {id}")

def save_products_to_file(list_of_products, file_name):
...
def add_product(list_of_products):
    ...
    price = input("Pris: ")
    new_product = [id, title, brand, price]
    list_of_products.append(new_product)

def print_products(list_of_products, heading):
    """Skriver ut produkterna
    
    Args:
        list_of_products (list): Lista på produkterna
        heading (str) : Rubriken för produktlistan
    """
    header(heading)
...
def print_menu():
    ...
    print("4) Sök efter en produkter")
    print("5) Spara produkter")
    print("0) Avsluta")

def read_products_from_file(file_name):
    """Läser in produkter från en fil och returnerar en nästlad lista, i följande format
...
```
1;iPhone17;Apple;16000
2;Pixel 10;Google;9900
3;iPad Pro;Apple;12000
5;Robotdamsugare;iRobot;7000
6;Tvättmaskin;Bosch;15000
