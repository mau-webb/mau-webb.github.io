...
- [FastAPI](https://fastapi.tiangolo.com/) är ett nytt och snabbt ramverk för att bygga webbappar. Det är enkelt att använda och har bra dokumentation. Installera med `pip install fastapi` och `pip install uvicorn`. **Obs**: Det kan vara lite knepigt att starta första gången, på grund av uvicorn, men om ni vill ta er an en extra utmaning så kommer detta ramverket ge er en del fördelar längre fram i andra kurser som webbtjänster (som ni läser i två:an på IA programmet). Se [denna guide](https://fastapi.tiangolo.com/tutorial/first-steps/) för att komma igång.
